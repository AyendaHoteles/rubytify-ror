...
RSpec.describe Spotify::ComunicationService, type: :service do
  ...
  describe ".get_artist" do
    ...
    context "when artist can be found" do
      ...
      before do 
        allow(described_class).to receive(:get_artist_id).and_return("3MqjsWDLhq8SyY6N3PE8yW")
        allow(HTTParty).to receive(:get).and_return(OpenStruct.new({code: 200, body: spotify_artist_response.to_json})) 
      end

      it "returns spotify artist info" do
        expect(subject.body).to include_json(spotify_artist_response)
      end
    end
